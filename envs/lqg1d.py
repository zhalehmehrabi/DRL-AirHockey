...
#classic_control
from gym.envs.registration import EnvSpec, register, spec
try:
    spec('LQG1D-v0')
except:
    register(
...
class LQG1D(gym.Env):
    ...
    def __init__(self, discrete_reward=False, sigma_noise=0.1):
        self.horizon = 20
        self.gamma = 0.99
    ...
    def step(self, action, render=False):
        ...
        return self.get_state(), -normalized_cost.item(), False, {}

    def get_opt_action(self, x):
    ...
    def reset(self, random_start=True):
        ...
        noise = np.random.randn() * self.sigma_noise if random_start else 0
        
        if np.random.rand() > .5: 
            self.state = self.max_pos + noise
        else:
            self.state = -self.max_pos + noise
...
if __name__ == '__main__':
    env = LQG1D(discrete_reward=False, sigma_noise=0.15)
    ...
    # print('----Qf----')
    # for a in np.linspace(-4,4,41):
    #     print(env.computeQFunction(0, a, K, 1, n_random_xn=100))
